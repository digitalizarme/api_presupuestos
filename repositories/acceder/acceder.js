...
module.exports = async(data) => {
    ...
    if (_.get(data, 'email') && _.get(data, 'contrasena')) {
        ...
        throw {status: 500, message: "No Recibimos el email o la contraseña"};
    }
    if (!user) {
        throw {status: 500, message: "Email o Contraseña incorrecta, favor intentar nuevamente"};
    }

    if (!user.b_activo) {
        throw {status: 500, message: "Este usuario no esta activo en el sistema"};
    }

    const token = createJwtToken({username: user.email, id: user.id});
