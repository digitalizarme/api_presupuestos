...
module.exports = async (query) => {
    ...
    const params = {
        ...
        attributes: { 
            include: [
                [sequelize.literal(`CASE WHEN b_activo THEN 'SÍ' ELSE 'NO' END`), `c_activo`]
                ,[sequelize.literal(`CASE WHEN b_cliente THEN 'SÍ' ELSE 'NO' END`), `c_cliente`]
                ,[sequelize.literal(`CASE WHEN b_comisionista THEN 'SÍ' ELSE 'NO' END`), `c_comisionista`]
                ,[sequelize.literal(`CASE WHEN b_funcionario THEN 'SÍ' ELSE 'NO' END`), `c_funcionario`]
                ,[sequelize.literal(`CASE WHEN b_usuario THEN 'SÍ' ELSE 'NO' END`), `c_usuario`]
            ] 
        },
    }
    // const newTotal = {
